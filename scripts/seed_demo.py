...
if __name__ == "__main__":
    main()"""Seed demo data for development."""
from __future__ import annotations
...
def seed_admin(session) -> User:
    admin = session.query(User).filter(User.email == settings.admin_default_email).first()
    if admin:
        return admin
    admin = User(
        email=settings.admin_default_email,
    ...
    session.add(admin)
    session.flush()
    session.add(TwoFAMethod(user_id=admin.id, type=TwoFAMethodType.EMAIL, secret="seed"))
    return admin


def seed_users(session) -> tuple[User, User]:
    seller = session.query(User).filter(User.email == "seller@c2c.local").first()
    if seller is None:
        seller = User(
            email="seller@c2c.local",
            password_hash=hash_password("seller123"),
        )
        session.add(seller)
        session.flush()
        session.add(TwoFAMethod(user_id=seller.id, type=TwoFAMethodType.EMAIL, secret="seed"))
    buyer = session.query(User).filter(User.email == "buyer@c2c.local").first()
    if buyer is None:
        buyer = User(
            email="buyer@c2c.local",
            password_hash=hash_password("buyer123"),
        )
        session.add(buyer)
        session.flush()
        session.add(TwoFAMethod(user_id=buyer.id, type=TwoFAMethodType.EMAIL, secret="seed"))
        session.add(
            Membership(
                user_id=buyer.id,
...
def seed_admin_config(session) -> None:
    config = session.query(AdminConfig).first()
    if config is None:
        config = AdminConfig(commission_rate=Decimal("0.05"), cashback_default=Decimal("0.02"))
        session.add(config)
    ...
    coupon = session.query(Coupon).filter(Coupon.code == "WELCOME10").first()
    if coupon is None:
        coupon = Coupon(
            code="WELCOME10",
            status=CouponStatus.ACTIVE,
            min_revenue=Decimal("50.00"),
            min_sales=0,
            discount_amount=Decimal("10.00"),
        )
        session.add(coupon)
...
def seed_product(session, seller: User) -> None:
    product = session.query(Product).filter(Product.title == "Vintage Camera").first()
    if product is None:
        product = Product(
            seller_id=seller.id,
            title="Vintage Camera",
        ...
        session.add(product)
        session.flush()
        tag = Tag(name="camera", status=TagStatus.APPROVED)
        session.add(tag)
        session.flush()
        product.tags.append(tag)
...
def main() -> None:
    ...
    seller, buyer = seed_users(session)
    seed_admin_config(session)
        seed_product(session, seller)
        session.commit()
        _ = admin, buyer  # quiet lint for unused demo variables
